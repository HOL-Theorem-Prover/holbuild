...
if [[ -d "$HOLBUILD_CACHE/locks" ]] && find "$HOLBUILD_CACHE/locks" -mindepth 1 -maxdepth 1 -type d -name 'action-*.lock' | grep -q .; then
  echo "stale action cache lock directory left behind" >&2
  exit 1
fi
